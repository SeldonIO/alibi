...
```python
perturb_sentence_similarity(present: tuple, n: int, sample_proba: float = 0.5, forbidden: frozenset = frozenset(), forbidden_tags: frozenset = frozenset({'PRP$'}), forbidden_words: frozenset = frozenset({'be'}), temperature: float = 1.0, pos: frozenset = frozenset({'VERB', 'ADP', 'NOUN', 'ADJ', 'DET', 'ADV'}), use_proba: bool = False, kwargs) -> Tuple[numpy.ndarray, numpy.ndarray]
```
...
| `pos` | `frozenset` | `frozenset({'VERB', 'ADP', 'NOUN', 'ADJ', 'DET', 'ADV'})` | POS that can be changed during perturbation. |
