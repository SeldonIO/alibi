...
class PtCounterfactualRLDataset(CounterfactualRLDataset, Dataset):
    ...
    def __init__(self,
        ...
        self.y_m = PtCounterfactualRLDataset.predict_batches(x=self.x_orig,
                                                             predict_func=self.predict_func,
                                                             batch_size=self.batch_size)

        # Preprocess the input data.
        self.x = self.preprocessor(self.x_orig)
    ...
    def __getitem__(self, idx) -> Dict[str, np.ndarray]:
        ...
        c = self.conditional_func(self.x_orig[idx:idx+1])
        if c is not None:
...
class PtCounterfactualRLBaseBackend(CounterfactualRLBaseBackend):
    ...
    def update_actor_critic(ae: nn.Module,
        ...
        z_cf_tgt = PtCounterfactualRLBaseBackend.encode(x=x_cf, ae=ae, device=device)
    ...
    def to_numpy(x: Optional[Union[List, np.ndarray, torch.Tensor]]) -> Optional[Union[List[np.ndarray], np.ndarray]]:
        ...
        if x is not None:
            ...
            if isinstance(x, list):
                return [PtCounterfactualRLBaseBackend.to_numpy(e) for e in x]
