...
class AnchorImage(object):
    ...
    def get_sample_fn(self, image: np.ndarray, p_sample: float = 0.5) -> Tuple[np.ndarray, Callable]:
        ...
        def sample_fn_image(
            ...
            data = np.random.choice([0, 1], num_samples * n_features, p=[p_sample, 1 - p_sample]).reshape(
    ...
    def explain(
        ...
        **kwargs: Any
        ...
        exp = AnchorBaseBeam.anchor_beam(
            sample_fn, delta=delta, epsilon=tau, batch_size=batch_size, desired_confidence=threshold, **kwargs
        )  # type: Any
        exp["instance"] = image
        ...
        for ex in exp["examples"]:
            for opt in cover_options:
                tmp = [
                    AnchorImage.overlay_mask(image, segments, list(np.where(ex[opt][i] == 1)[0]))
    ...
    @staticmethod
    def overlay_mask(
        image: np.ndarray, segments: np.ndarray, mask_features: list, scale: tuple = (0, 255)
    ) -> np.ndarray:
        """
