...
class PtCounterfactualRLTabularBackend(CounterfactualRLTabularBackend,
                                       PtCounterfactualRLBaseBackend):
    ...
    def sparsity_loss(x_ohe_hat_split: List[torch.Tensor],
        ...
        x_ohe_num_split, x_ohe_cat_split = PtCounterfactualRLTabularBackend.split_ohe(x_ohe=x_ohe,
                                                                                      category_map=category_map)

        # sample differentiable output
        x_ohe_hat_split = PtCounterfactualRLTabularBackend.sample_differentiable(x_ohe_hat_split=x_ohe_hat_split,
                                                                                 category_map=category_map)
        ...
        if len(x_ohe_num_split) > 0:
            offset = 1
            num_loss = torch.mean(PtCounterfactualRLTabularBackend.l1_loss(input=x_ohe_hat_split[0],
        ...
        if len(x_ohe_cat_split) > 0:
            for i in range(len(x_ohe_cat_split)):
                ...
                cat_loss += torch.sum(PtCounterfactualRLTabularBackend.l0_ohe(input=x_ohe_hat_split[i + offset],
