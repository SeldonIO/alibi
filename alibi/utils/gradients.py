...
def perturb(X: np.ndarray, eps: Union[float, np.ndarray] = 1e-08, proba: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def num_grad_batch(
    func: Callable, X: np.ndarray, args: Tuple = (), eps: Union[float, np.ndarray] = 1e-08
) -> np.ndarray:
    """
    ...
    grad_numerator = np.reshape(
        np.reshape(grad_numerator, (batch_size, -1)), (batch_size, preds.shape[1], -1), order="F"
    )  # NxPxF
