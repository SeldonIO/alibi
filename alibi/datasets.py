...
__all__ = ["fetch_adult", "fetch_imagenet", "fetch_movie_sentiment"]
...
def imagenet(
    ...
    warnings.warn(
        "`imagenet` is deprecated and will be removed soon, use `fetch_imagenet` instead",
        DeprecationWarning,
        stacklevel=2,
    )
...
def movie_sentiment():
    ...
    warnings.warn(
        "`movie_sentiment` is deprecated and will be removed soon, use `fetch_movie_sentiment` instead",
        DeprecationWarning,
        stacklevel=2,
    )
...
def fetch_adult(
    features_drop: list = None, return_X_y: bool = False, url_id: int = 0
) -> Union[Bunch, Tuple[np.ndarray, np.ndarray]]:
    """
