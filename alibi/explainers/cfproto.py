...
class CounterFactualProto:
    def __init__(
        ...
        if is_model:  # Keras or TF model
            self.model = True
            self.classes = self.predict.predict(np.zeros(shape)).shape[  # type: ignore
                1
            ]
        ...
        if self.is_cat:
            ...
            def body_ohe_to_num(icol, iohe, icat, adv_to_map, adv_map, map_cols):
                ...
                # nb of categories
                v = tf.cond(eq_any_true, lambda: tf.shape(self.map_cat_to_num[icat])[0], lambda: tf.constant(1))
            ...
            def apply_map(adv_to_map, to_num):
                ...
                if to_num:  # map from categorical to numerical values
                    _, _, _, _, adv_map, _ = tf.while_loop(
                        cond_loop, body_to_num, loop_vars, parallel_iterations=1, back_prop=True
                    )
                else:  # map from numerical to categorical values
                    _, _, _, _, adv_map, _ = tf.while_loop(
                        cond_loop, body_to_cat, loop_vars, parallel_iterations=1, back_prop=True
                    )
        ...
        with tf.name_scope("shrinkage_thresholding") as scope:
            cond = [
                tf.cast(tf.greater(tf.subtract(self.adv_s, self.orig), self.beta), tf.float32),
            ...
            self.assign_adv = (
                tf.multiply(cond[0], upper) + tf.multiply(cond[1], self.orig) + tf.multiply(cond[2], lower)
            )
        ...
        with tf.name_scope("loss_combined") as scope:
            ...
            self.loss_total = (
                self.loss_attack + self.loss_l2 + self.loss_ae + tf.multiply(self.beta, self.loss_l1) + self.loss_proto
            )
    ...
    def fit(
        ...
        if self.is_cat:  # compute distance metrics for categorical variables
            ...
            if d_type not in ["abdm", "mvdm", "abdm-mvdm"]:
                raise ValueError(
                    'd_type needs to be "abdm", "mvdm" or "abdm-mvdm". ' "{} is not supported.".format(d_type)
                )
            ...
            if d_type == "abdm-mvdm":
                ...
                self.d_abs, self.feature_range = multidim_scaling(
                    d_pair,
                    n_components=2,
                    use_metric=True,
                    feature_range=self.feature_range,
    ...
    def get_gradients(self, X: np.ndarray, Y: np.ndarray, grads_shape: tuple, cat_vars_ord: dict = None) -> np.ndarray:
        """
        ...
        dp_dx = np.reshape(np.reshape(dp_dx, (X.shape[0], -1)), (X.shape[0], preds.shape[1], -1), order="F") / (
            2 * self.eps[1]
        )  # NxPxF
        ...
        if self.enc_model:
            ...
            for c, v in class_dict.items():
                ...
                if k is None:
                    ...
                    dist_k = np.linalg.norm(X_enc.reshape(X_enc.shape[0], -1) - v.reshape(v.shape[0], -1), axis=1)
            ...
            for i in range(self.max_iterations):
                ...
                if not self.model and (self.c_init != 0.0 or self.c_steps > 1):
                    ...
                    if i % self.update_num_grad == 0 and i > 0:  # compute numerical gradients
                        ...
                        grads_num = (
                            self.get_gradients(
                                X_der_batch, Y, cat_vars_ord=self.cat_vars_ord, grads_shape=pert_shape[1:]
                            )
                            * c
                        )
                        grads_num_s = (
                            self.get_gradients(
                                X_der_batch_s, Y, cat_vars_ord=self.cat_vars_ord, grads_shape=pert_shape[1:]
                            )
                            * c
                        )
                        # clip gradients
                ...
                if self.model:
                    loss_tot, loss_attack, loss_l1_l2, pred_proba, adv = self.sess.run(
                        [self.loss_total, self.loss_attack, self.l1_l2, self.pred_proba, self.adv]
                    )
                else:
                ...
                if verbose and i % print_every == 0:
                    ...
                    print(
                        "Target proba: {:.2f}, max non target proba: {:.2f}".format(target_proba, nontarget_proba_max)
                    )
                    ...
                    if not self.model:
                        print(
                            "Gradient numerical attack min/max: {:.3f}/{:.3f}".format(grads_num.min(), grads_num.max())
                        )
                        print(
